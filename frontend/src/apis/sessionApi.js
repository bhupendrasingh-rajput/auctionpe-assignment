...
const url = `${import.meta.env.VITE_APP_BACKEND_URL}/api/sessions`;

const startSession = async (token) => {
    try {
        const response = await axios.post(`${url}/start-session`, {}, {
...
const endSession = async (token, sessionId) => {
    try {
        const response = await axios.post(`${url}/end-session`, { sessionId }, {
            headers: { Authorization: token }
        });
        return response.data;
    } catch (error) {
